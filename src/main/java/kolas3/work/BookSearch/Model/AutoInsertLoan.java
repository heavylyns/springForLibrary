...
public class AutoInsertLoan {
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	public String insert(String selbook, String userid, String msa) throws SQLException {
		...
		if (loanenddayuse.equals("Y")) {
			sqlmanager.connect();
			String loanenddayqry = "select to_date(SYSDATE + " + deldate + ", 'yyyy/mm/dd') from dual";
			...
			if (holidayuse.equals("N")) {
				...
				while (rs.next()) {
					...
					logger.debug(String.valueOf(holidayyear.get(r)));
					logger.debug(String.valueOf(holidaymonth.get(r)));
					logger.debug(String.valueOf(holidaydays.get(r)));
					r++;
				}
				sqlmanager.disconnect();
		...
		if (!selbook.equals("")) {
			...
			bookinfoqry = bookinfoqry + "species_key, working_status, separate_shelf_code, reg_no from " + bookjong
					+ " where rec_key = '" + selbook + "'";
			...
			if (holidayuse.equals("Y")) {
				...
				while (rs.next()) {
					...
					logger.debug(String.valueOf(subholidayyear.get(r)));
					logger.debug(String.valueOf(subholidaymonth.get(r)));
					logger.debug(String.valueOf(subholidaydays.get(r)));
					r++;
				}
				sqlmanager.disconnect();
			...
			sqlmanager.connect();
			String loaninsertqry = "insert into LS_WORK_T01 (rec_key, book_key, shelf_loc_code, publish_form_code, class_no_type, ";
			...
			loaninsertqry = loaninsertqry + "separate_shelf_code, reg_no, title, author, publisher, call_no,first_work";
			...
			loaninsertqry = loaninsertqry + "'" + separate_shelf_code2 + "', '" + reg_no + "', '" + idx_title + "', '"
					+ idx_author + "', '" + idx_publisher + "','" + call_no + "','" + logdata + "'";
			loaninsertqry = loaninsertqry + ")";
